...
count(distinct case demands.receiver_account_type when 'Self' then dispatch_id else null end) as num_self_dispatches,
count(distinct case demands.receiver_account_type when 'Partner' then dispatch_id else null end) as num_partner_dispatches,
...
group by demands.annual_year, demands.receiver_center_name,demands.internal_demand
